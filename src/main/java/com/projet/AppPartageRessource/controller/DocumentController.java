...
public class DocumentController {
 ...
 private final  DocumentService docService ;
    ...
    public String indexUserDoc( HttpSession session, Model model){
        ...
        model.addAttribute("documents",docService.findUserDoc( statut,((Utilisateur)session.getAttribute("loggedInUser")).getId()));
    ...
    public String createDoc(@ModelAttribute("doc") Document doc, HttpSession session, ModelAndView model,
        ...
        if(createOrUpdate!=null && (createOrUpdate.equals("update") || createOrUpdate.equals("create"))) {
            ...
            model.addObject("documents", docService.findUserDoc(Statut.Disponible, ((Utilisateur) session.getAttribute("loggedInUser")).getId()));
    ...
    public String archives(HttpSession session, Model model){
        ...
        List<Document> lisUserDoc = docService.findUserDocArchive(Statut.Archive,((Utilisateur)session.getAttribute("loggedInUser")).getId());
        model.addAttribute("documents",lisUserDoc);
        return "archiveDoc";
    ...
    public String desarchive(@RequestParam(name = "doc_id",required = false)Integer docId){

        docService.DesArchive(docId);
